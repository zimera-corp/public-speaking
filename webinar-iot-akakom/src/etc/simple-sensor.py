...
# Original source code: Steve Cope (www.steves-internet-guide.com)
...
# loop time when sensor publishes in verbose
options["interval"]=10

# in non chatty mode publish status at this interval if 0 then ignore:
options["interval_pub"]=300
...
# loop time when sensor publishes
interval=2

# how often to publish if status is unchanged
sensor_pub_interval=300
...
if options["sensor_type"]=="light":
    states=["1","0"] #possible sensor states
else:
    states=["1","0"] #possible sensor states    
    ...
    while run_flag:
        ...
        if not client.connected_flag:
            if Connect(client,options["broker"],options["port"],\
                ...
                run_flag=False #break
        #subscribes to control in on_connect calback
